...
1. **название правила** (например `CPU > 80%`);
2. **targetTypePrefix** — тип целей, напр. `VM` → все параметры `VM_*`;
3. **PromQL-шаблон** с плейсхолдером `$VM` (значение конкретного параметра);
4. **условие нарушения** (`ThresholdCondition`, напр. `value > 80`).

### Матчинг и выполнение запросов

Пример: `VM_Kafka_GW=server1`, `VM_Kafka_GW=server2`, `VM_Postgre_ASD=server3` + 2 VM-плагина (CPU, RAM)
→ **6** запросов к VictoriaMetrics (каждый параметр × каждое правило).

Статусы результата:

| Статус | Смысл |
|--------|--------|
| OK | превышения не было |
| Fail | превышение было |
...
Сейчас: оболочка с loader + группы по типу ПО, внутри строки параметр/значение/правило/статус (`OK` / `Fail` / `No Data` / `Skip`).
Целевой вид: светофоры / индикаторы. Детальный макет — отдельно.
