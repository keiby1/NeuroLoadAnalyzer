...
| `victoriametrics.cpu-rate-window` | `5m` | Окно `rate()` для CPU контейнеров (K8S) |
| `victoriametrics.aggregation-method` | `sum_then_percent` | Агрегация CPU/RAM % по деплойменту |
...
|-----|--------|-----------|
| `Тип_Софт_Назначение` | `VM_Kafka_GW=host:9100` | VM-плагины; группа «Виртуальные сервера» |
| `Тип_Софт_Назначение_opt` | `VM_Kafka_GW_opt=host` | те же плагины; в дереве с пометкой opt; **не** влияет на rollup/вердикт |
| `k8s_namespace` | `k8s_namespace=payments` (можно несколько) | inventory Deployments/StatefulSets + K8S-плагины; группа «K8S» |

Группа в отчёте появляется только если по ней есть результаты плагинов (нет параметров → нет раздела).
...
Узлы optional (`*_opt`) видны в UI/JSON со своим статусом, но **исключаются** из aggregate родителя (если только opt — родитель OK).
...
VM/K8S **CPU**: `rate[5m]` / `rate[1m]` + `max_over_time` (bands 78/80).
VM дополнительно: **CPU spike [irate]** (`max_over_time` step 15s) и **Unexpected reboot** (`changes(node_boot_time_seconds)`).
...
Иерархия:
`Тип` (`VM` → «Виртуальные сервера», `K8S` → «K8S», маппинг `TargetTypeLabels`)
